...
function ESP:MakeText(parent)
    ...
    l.FontFace = Fonts[self.Config.Text.Font]
    l.Text = ""
    l.Parent = parent
    return l
end
...
function ESP:CreateBox(plr)
    ...
    gui.Name = "ESP_Box_"..plr.Name; gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; gui.Parent = CoreGui
    ...
    sgrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, self.Config.Box.Gradient.Color1),
...
function ESP:MakeBar(plr, name)
    local cfg = name == "Health" and self.Config.Bars.Health or self.Config.Bars.Armor
    local gui = Instance.new("ScreenGui")
    gui.Name = "ESP_"..name.."_"..plr.Name; gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling; gui.Parent = CoreGui
    local outline = Instance.new("Frame")
    outline.BackgroundColor3 = Color3.new(0,0,0); outline.BorderSizePixel = 0; outline.Parent = gui
    local fill = Instance.new("Frame")
    fill.BackgroundTransparency = 0; fill.BorderSizePixel = 0; fill.Parent = outline
    ...
    grad.Rotation = 90; grad.Parent = fill
...
function ESP:MakeHighlight(plr, char)
    ...
    h.FillColor = self.Config.Highlight.Color; h.OutlineColor = self.Config.Highlight.Outline
    ...
    h.Adornee = char; h.Parent = CoreGui
    if self.Cache[plr] then self.Cache[plr].Highlight = h end
end
...
function ESP:MakeChams(plr, char)
    ...
    local list = {}; local z = self.Config.Chams.BehindWalls and 1 or -1
    for _, p in ipairs(GetBodyParts(char)) do
        local b = Instance.new("BoxHandleAdornment")
        b.Adornee = p; b.Color3 = self.Config.Chams.Color; b.ZIndex = z; b.AlwaysOnTop = self.Config.Chams.BehindWalls
        b.Size = p.Size + ChamsOffset; b.Transparency = 0.5; b.Parent = CoreGui; list[#list+1] = b
    end
    if self.Cache[plr] then self.Cache[plr].Chams = list end
end

function ESP:RemoveChams(plr)
    local c = self.Cache[plr]
    if c and c.Chams then for _, v in ipairs(c.Chams) do pcall(v.Destroy, v) end; c.Chams = nil end
end
...
function ESP:ApplyMaterial(plr, char)
    ...
    task.wait(0.2)
    local mat = self.Config.Material.Material; local col = self.Config.Material.Color
    for _, p in ipairs(GetBodyParts(char)) do
    ...
    local function strip(o, a, p)
        if o and o[p] ~= "" then o:SetAttribute(a, o[p]); o[p] = "" end
    end
...
function ESP:RevertMaterial(plr, char)
    ...
    local function restore(o, a, p)
        if o then local x = o:GetAttribute(a); if x then o[p] = x; o:SetAttribute(a, nil) end end
    end
...
function ESP:AddPlayer(plr)
    ...
    c.CharConn = plr.CharacterAdded:Connect(function(nc)
        c.Character = nc
        if self.Config.Highlight.Enabled then self:MakeHighlight(plr, nc) end
        if self.Config.Chams.Enabled then self:MakeChams(plr, nc) end
        if self.Config.Material.Enabled then task.spawn(self.ApplyMaterial, self, plr, nc) end
    end)
    self.Cache[plr] = c
    local char = plr.Character
    if char then
        c.Character = char
        if self.Config.Highlight.Enabled then self:MakeHighlight(plr, char) end
        if self.Config.Chams.Enabled then self:MakeChams(plr, char) end
        if self.Config.Material.Enabled then task.spawn(self.ApplyMaterial, self, plr, char) end
    end
end
...
function ESP:Update()
    ...
    local now = tick(); local dt = now - self.FrameTick; self.FrameTick = now
    ...
    for plr, c in pairs(self.Cache) do
        ...
        local center, sz3 = CustomBounds(char)
        ...
        -- Highlight
        if cfg.Highlight.Enabled then
            if not c.Highlight then self:MakeHighlight(plr, char) end
            if c.Highlight then
                c.Highlight.FillColor = cfg.Highlight.Color; c.Highlight.OutlineColor = cfg.Highlight.Outline
                c.Highlight.DepthMode = cfg.Highlight.BehindWalls and Enum.HighlightDepthMode.AlwaysOnTop or Enum.HighlightDepthMode.Occluded
            end
        ...
        if cfg.Chams.Enabled then
            ...
            local z = cfg.Chams.BehindWalls and 1 or -1
            for _, v in ipairs(c.Chams or {}) do v.Color3 = cfg.Chams.Color; v.ZIndex = z; v.AlwaysOnTop = cfg.Chams.BehindWalls end
        else self:RemoveChams(plr) end

        -- Material
        if cfg.Material.Enabled and not c.MatDone then
            task.spawn(self.ApplyMaterial, self, plr, char); c.MatDone = true
        elseif not cfg.Material.Enabled and c.MatDone then
            self:RevertMaterial(plr, char); c.MatDone = false
        end
        ...
        if cfg.Box.Enabled then
            ...
            if cfg.Box.Filled.Enabled then
                ...
                bx.FillGrad.Color = ColorSequence.new({
                    ...
                    ColorSequenceKeypoint.new(1, cfg.Box.Filled.Gradient.Color3),
                })
                if cfg.Box.Filled.Gradient.Rotation.Moving.Enabled then
                    self.RotationAngle = (self.RotationAngle + dt * cfg.Box.Filled.Gradient.Rotation.Moving.Speed) % 360
                    bx.FillGrad.Rotation = self.RotationAngle
                else
                    bx.FillGrad.Rotation = cfg.Box.Filled.Gradient.Rotation.Amount
                end
            else
        ...
        -- Bars
        local bw = cfg.Bars.Width; local bh = scale.Y
        local function drawBar(bar, bcfg, val, maxVal, lastKey, offX)
            if not bar then return false end
            if bcfg.Enabled then
                local pct = math.clamp(val / maxVal, 0, 1)
                local lerp = c[lastKey] or pct; lerp = lerp + (pct - lerp) * cfg.Bars.Lerp; c[lastKey] = lerp
                local x = pos.X - (bw + 4) - offX
                local out = bar.Outline; local fill = bar.Fill
                out.Visible = true
                if cfg.Bars.Resize then
                    local ch = math.max(bh * lerp, 2)
                    out.Position = UDim2.fromOffset(x-1, by+bh-ch-1); out.Size = UDim2.fromOffset(bw+2, ch+2)
                    fill.Visible = true; fill.Position = UDim2.fromOffset(1, 1); fill.Size = UDim2.fromOffset(bw, ch)
                else
                    out.Position = UDim2.fromOffset(x-1, by-1); out.Size = UDim2.fromOffset(bw+2, bh+2)
                    fill.Visible = true; fill.Position = UDim2.fromOffset(1, (1-lerp)*bh+1); fill.Size = UDim2.fromOffset(bw, lerp*bh)
                end
                out.BackgroundTransparency = 0.2
                if bar.Gradient then
                    bar.Gradient.Color = cfg.Bars.Type == "Gradient"
        ...
        local hpVis = drawBar(c.HpBar, cfg.Bars.Health, hum.Health, hum.MaxHealth, "HpLast", 0)
        if cfg.Bars.Armor.Enabled then
            local be = char:FindFirstChild("BodyEffects"); local av = be and be:FindFirstChild("Armor"); local val = av and av.Value or 0
            if cfg.Bars.Armor.Armored and val <= 0 then
                c.ArmBar.Outline.Visible = false; c.ArmBar.Fill.Visible = false
            else
                drawBar(c.ArmBar, cfg.Bars.Armor, val, 130, "ArmLast", hpVis and (bw*2+6+2) or 0)
            end
        else
            c.ArmBar.Outline.Visible = false; c.ArmBar.Fill.Visible = false
        end
        ...
        -- Text
        local cx = pos.X + scale.X/2; local cy = pos.Y; local ts = cfg.Text
        local font = Fonts[cfg.Text.Font] or Fonts.GothamBold
        ...
        else c.NameLbl.Visible = false end

        local wepY, distY
        if ts.Weapon.Enabled and ts.Distance.Enabled then
            wepY = cy + scale.Y + 5 - inset; distY = cy + scale.Y + 15 - inset
        elseif ts.Weapon.Enabled then wepY = cy + scale.Y + 5 - inset
        elseif ts.Distance.Enabled then distY = cy + scale.Y + 5 - inset end
        ...
        else c.WeapLbl.Visible = false end
        ...
        else c.DistLbl.Visible = false end
    end
end
...
function ESP:BuildUI(Tab)
    ...
    L:AddSlider("FillRot", { Text = "Fill Rotation", Default = 45, Min = 0, Max = 360, Rounding = 0, Callback = function(v) self.Config.Box.Filled.Gradient.Rotation.Amount = v end })
    local fm = L:AddToggle("FillMove", { Text = "Animate Rotation", Default = false, Callback = function(v) self.Config.Box.Filled.Gradient.Rotation.Moving.Enabled = v end })
    fm:AddSlider("FillSpeed", { Text = "Speed", Default = 300, Min = 10, Max = 1000, Rounding = 0, Callback = function(v) self.Config.Box.Filled.Gradient.Rotation.Moving.Speed = v end })
    L:AddDivider()

    local ht = L:AddToggle("ESP_HL", { Text = "Highlight", Default = false, Callback = function(v)
        self.Config.Highlight.Enabled = v
        if not v then for p in pairs(self.Cache) do self:RemoveHighlight(p) end end
    end })
    ht:AddColorPicker("HLCol", { Default = self.Config.Highlight.Color, Callback = function(v) self.Config.Highlight.Color = v end })
    ht:AddColorPicker("HLOut", { Default = self.Config.Highlight.Outline, Callback = function(v) self.Config.Highlight.Outline = v end })
    L:AddToggle("HLWalls", { Text = "Behind Walls", Default = false, Callback = function(v) self.Config.Highlight.BehindWalls = v end })

    local ct = L:AddToggle("ESP_Ch", { Text = "Chams", Default = false, Callback = function(v)
        self.Config.Chams.Enabled = v
        if not v then for p in pairs(self.Cache) do self:RemoveChams(p) end end
    end })
    ct:AddColorPicker("ChCol", { Default = self.Config.Chams.Color, Callback = function(v) self.Config.Chams.Color = v end })
    L:AddToggle("ChWalls", { Text = "Behind Walls", Default = false, Callback = function(v) self.Config.Chams.BehindWalls = v end })
    L:AddDivider()

    local mt = L:AddToggle("ESP_Mat", { Text = "Material Override", Default = false, Callback = function(v)
        self.Config.Material.Enabled = v
        if not v then for p, c in pairs(self.Cache) do if c.MatDone then self:RevertMaterial(p, c.Character); c.MatDone = false end end end
    end })
    mt:AddColorPicker("MatCol", { Default = self.Config.Material.Color, Callback = function(v) self.Config.Material.Color = v end })
    L:AddDropdown("MatType", { Values = {"ForceField","Neon","Glass","SmoothPlastic"}, Default = "ForceField", Text = "Material", Callback = function(v) self.Config.Material.Material = Enum.Material[v] end })

    R:AddDivider()
    local nt = R:AddToggle("ESP_Nm", { Text = "Name", Default = false, Callback = function(v) self.Config.Text.Name.Enabled = v end })
    nt:AddColorPicker("NmCol", { Default = self.Config.Text.Name.Color, Callback = function(v) self.Config.Text.Name.Color = v end })
    R:AddDropdown("NmMode", { Values = {"DisplayName","Username"}, Default = "DisplayName", Text = "Type", Callback = function(v) self.Config.Text.Name.Type = v end })
    R:AddDropdown("NmCase", { Values = {"lowercase","UPPERCASE","Normal"}, Default = "lowercase", Text = "Case", Callback = function(v) self.Config.Text.Name.Casing = v end })

    local wt = R:AddToggle("ESP_Wp", { Text = "Weapon", Default = false, Callback = function(v) self.Config.Text.Weapon.Enabled = v end })
    wt:AddColorPicker("WpCol", { Default = self.Config.Text.Weapon.Color, Callback = function(v) self.Config.Text.Weapon.Color = v end })
    R:AddDropdown("WpCase", { Values = {"lowercase","UPPERCASE","Normal"}, Default = "lowercase", Text = "Case", Callback = function(v) self.Config.Text.Weapon.Casing = v end })

    local dt = R:AddToggle("ESP_Dt", { Text = "Distance", Default = false, Callback = function(v) self.Config.Text.Distance.Enabled = v end })
    dt:AddColorPicker("DtCol", { Default = self.Config.Text.Distance.Color, Callback = function(v) self.Config.Text.Distance.Color = v end })
    R:AddDropdown("DtCase", { Values = {"lowercase","UPPERCASE","Normal"}, Default = "lowercase", Text = "Case", Callback = function(v) self.Config.Text.Distance.Casing = v end })
    R:AddDropdown("ESPFont", { Values = {"SourceSans","SourceSansBold","Gotham","GothamBold","Minecraft","Cartoon"}, Default = "GothamBold", Text = "Font", Callback = function(v) self.Config.Text.Font = v end })
    R:AddSlider("TxtSize", { Text = "Text Size", Default = 10, Min = 6, Max = 20, Rounding = 0, Callback = function(v) end })
    R:AddDivider()

    local hb = R:AddToggle("HP_Hp", { Text = "Health Bar", Default = false, Callback = function(v) self.Config.Bars.Health.Enabled = v end })
    hb:AddColorPicker("Hp1", { Default = self.Config.Bars.Health.Color1, Callback = function(v) self.Config.Bars.Health.Color1 = v end })
    hb:AddColorPicker("Hp2", { Default = self.Config.Bars.Health.Color2, Callback = function(v) self.Config.Bars.Health.Color2 = v end })
    hb:AddColorPicker("Hp3", { Default = self.Config.Bars.Health.Color3, Callback = function(v) self.Config.Bars.Health.Color3 = v end })

    local ab = R:AddToggle("HP_Arm", { Text = "Armor Bar", Default = false, Callback = function(v) self.Config.Bars.Armor.Enabled = v end })
    ab:AddColorPicker("Ar1", { Default = self.Config.Bars.Armor.Color1, Callback = function(v) self.Config.Bars.Armor.Color1 = v end })
    ab:AddColorPicker("Ar2", { Default = self.Config.Bars.Armor.Color2, Callback = function(v) self.Config.Bars.Armor.Color2 = v end })
    ab:AddColorPicker("Ar3", { Default = self.Config.Bars.Armor.Color3, Callback = function(v) self.Config.Bars.Armor.Color3 = v end })
    ab:AddToggle("ArmOnly", { Text = "Show When Armored", Default = false, Callback = function(v) self.Config.Bars.Armor.Armored = v end })
    R:AddSlider("BarW", { Text = "Bar Width", Default = 2.5, Min = 1, Max = 6, Rounding = 1, Callback = function(v) self.Config.Bars.Width = v end })
    R:AddSlider("BarLerp", { Text = "Bar Smoothing", Default = 0.05, Min = 0.01, Max = 1, Rounding = 2, Callback = function(v) self.Config.Bars.Lerp = v end })
    R:AddToggle("BarResize", { Text = "Resize Bars", Default = false, Callback = function(v) self.Config.Bars.Resize = v end })
    R:AddDropdown("BarType", { Values = {"Gradient","Solid Color"}, Default = "Gradient", Text = "Bar Style", Callback = function(v) self.Config.Bars.Type = v end })
    R:AddDivider()
    ...
    en:AddKeyPicker("ESP_Key", { Text = "Toggle", Default = "None", Mode = "Toggle", SyncToggleState = true, Callback = function(v)
        self.Enabled = v
        if v then for _, p in ipairs(Players:GetPlayers()) do if p ~= Players.LocalPlayer then self:AddPlayer(p) end end else self:Clear() end
    end })
end

function ESP:StartConnections()
    self:StopConnections()
    self.Connections.PlayerAdded = Players.PlayerAdded:Connect(function(p)
        if p ~= Players.LocalPlayer and self.Enabled then self:AddPlayer(p) end
    end)
    self.Connections.PlayerRemoving = Players.PlayerRemoving:Connect(function(p)
        if p ~= Players.LocalPlayer then self:DeletePlayer(p) end
    end)
